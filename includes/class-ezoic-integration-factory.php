...
namespace Bloomly_Namespace {
	...
	if ( ! defined( 'EZOIC_INTEGRATION_VERSION' ) ) {
		define( 'EZOIC_INTEGRATION_VERSION', '1.2.1' ); // update version number
	}
	...
	class Ezoic_Integration_Factory {
		...
		public function NewEzoicIntegrator( $cacheType ) {
			...
			return new Ezoic_Integrator( $this->NewEzoicRequest(),
		...
		private function NewEzoicFilter() {
			if ( $this->cacheType != Ezoic_Cache_Type::NO_CACHE ) {
				...
				return new Ezoic_Integration_Cache_Filter( getallheaders(), false );
			}

			return new Ezoic_Integration_WP_Filter( getallheaders(), false );
		}
