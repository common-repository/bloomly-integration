...
namespace Bloomly_Namespace {
	...
	class Ezoic_Integration_Cache_Endpoints implements iEzoic_Integration_Endpoints {
		...
		private function getEndpointsFromServer() {
			$settings = array(
				...
				CURLOPT_HTTPHEADER     => array(
					'Referrer'        => "{$this->protocol}://" . $_SERVER["HTTP_HOST"],
		...
		private function curlRequestEndpointsFromEzoic() {
			...
			$endpoint_headers['Referrer']        = "{$this->protocol}://" . $_SERVER["HTTP_HOST"];
		...
		private function getEndpointsFromFile() {
			if ( file_exists( $this->filePath ) && is_readable( $this->filePath ) ) {
				...
				if ( is_array( $content ) && isset( $content["cache_time"] ) && isset( $content["endpoints"] ) ) {
		...
		private function storeEndpointsToFile() {
			$cacheContent = array( "cache_time" => $this->cacheTime, "endpoints" => $this->endpoints );
